...
# CORS: cho phép frontend ở http://localhost:3000 truy cập
app.add_middleware(
    CORSMiddleware,
...
def load_embeddings_from_json(
    ...
    if blacklist is None:
        blacklist = [
            ...
            "Picture", "case", "Pctv", "Strip", "Mp3", "EM60", "phone", "Speaker", "StarTech.com", "Kensington",
    ...
    if overwrite:
        ...
        print(f"✅ Overwrote JSON file '{"fff.json"}' with {'normalized' if normalize else 'original'} vectors.")
...
# def get_recommendations(user_profile: str, testing= True) -> dict:
#     """
#     Core logic:
#     1) Thử fetch search_history (sh) và view_history (vh) từ User Service.
#        Nếu lỗi (network, 404, v.v.), set sh, vh = [].
#     2) Lấy danh sách products từ Product Service.
#     3) Trả về dict: { recommendations, search_history, view_history }.
#     """
#     # 1) Lấy history từ User Service
    
#     try:
#         sh_resp = requests.get(
#             f"http://localhost:8003/users/user/history/search",
#             timeout=2  # gán timeout ngắn để dev nhanh thấy lỗi
#         )
#         vh_resp = requests.get(
#             f"http://localhost:8003/users/user/history/view",
#             timeout=2
#         )
#         # Nếu OK thì parse json, không thì trả về list rỗng
#         sh = sh_resp.json() if sh_resp.ok else []
#         vh = vh_resp.json() if vh_resp.ok else []
#     except Exception:
#         sh, vh = [], []
#     if testing:
#         # Nếu đang test, dùng dữ liệu mẫu
#         sh = t1
#         vh = t2
#     # 2) Lấy toàn bộ metadata sản phẩm (giả sử dạng list of dict)
#     try:
#         prods = requests.get("http://localhost:8001/products", timeout=20).json()
#     except Exception:
#         prods = []

#     # 3) Dummy: chọn top-k theo thứ tự, dùng 'title' làm tên hiển thị
#     k = 20
#     recs = []
#     blacklist = ["noble", "arm", "nook", "olympus", "bushnell", "california", "external", "truck","sawyer", "3g", "kindle", "db9", "yamakasi", "mygica", "bargain", "moleskine", "grade", "maxell", "ge", "palmone", "rca", "outdoor", "streambot", "aaa", "streambot", "lizone", "wpa4220kit", "allreli", "iclever", "34w", "inateck&reg;", "saicoo&trade;", "sharkk&reg;", "yens&reg;", "taotronics&reg;", "bolse&reg;", "jetech&reg;", "amp", "flash&trade;",
#                  "aerb&reg;", "supernight&reg;", "hypario&reg;", "mnxo&reg;", "digiyes&reg;", "bearextender", "dbpower", "avatar", "cable", "microphone", "radio", "ipod", "82mm", "dvd", "cd", "targus", "apc", "memorex", "diamond", "palm"]
#     print(f"length of products: {len(prods)}")
#     text2embed = []
#     #filterd_prods = [p for p in prods if p.get("title") not in blacklist]
#     new_prods = []
#     for p in prods:
#         # print(p,type(p))
#         ok = True
#         # Kiểm tra nếu title hoặc asin nằm trong blacklist
#         # Nếu có thì không thêm vào danh sách mới
        
#         u = (p.get("title",p.get("description",""))).lower() 
        
#         for v in blacklist:
#             if v in u:
#                 ok = False
#                 break
#         # print(f"checking {u} against blacklist: {ok}")
#         if ok:
#             print(u)
#             text2embed.append( p.get("title", "")+", "+p.get("description",""))
#             new_prods.append(p)
#     prods = new_prods
#     data = {
#         "text2embed": text2embed,
#     }
#     with open('output.json', 'w', encoding='utf-8') as f:
#         json.dump(data, f, ensure_ascii=False, indent=4)

#     print(f"length of filtered products: {len(prods)}")
#     for p in prods[:k]:
#         recs.append(p.get("title",p.get("description",0)))
#     # print("\n",str(sh_resp.content.decode("utf-8")),"\n")
#     return {
#         # "sh_resp": sh_resp.content.decode("utf-8"),
#         # "vh_resp": vh_resp.content.decode("utf-8"),
...
# ================================================================
def get_recommendations_real(user_profile: str, testing=True) -> dict:
    """
    Core logic:
    ...
    try:
        sh_resp = requests.get(
            f"http://localhost:8003/users/{user_profile}/history/search",
            timeout=2
        )
        vh_resp = requests.get(
            f"http://localhost:8003/users/{user_profile}/history/view",
            timeout=2
        )
        sh = sh_resp.json() if sh_resp.ok else []
        vh = vh_resp.json() if vh_resp.ok else []
    ...
    EMBED_PATH = "data/output_embeddings.json"
...
def recommend_endpoint(req: RecRequest):
    ...
    resp = get_recommendations_real(req.user_profile)
    return resp

if __name__ == "__main__":
    """
    Khi chạy `python app.py`, code dưới đây sẽ được thực thi.
    Thay `test_user` bằng user bạn muốn test; 
    In ra JSON để bạn thấy logic recommend hoạt động đúng.
    """
    if len(sys.argv) > 1:
        test_user = sys.argv[1]
    else:
        test_user = "some_demo_user"  # default nếu bạn không truyền param

    result = get_recommendations_real(test_user)
    # In ra dưới dạng JSON prettified (để dev xem dễ hơn)
