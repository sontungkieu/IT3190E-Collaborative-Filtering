...
class UserHistory(SQLModel, table=True):
    ...
    text: str
    type: str  # "search" or "view"
    created_at: datetime = Field(default_factory=datetime.utcnow)

# --- Database setup ---
engine = create_engine("sqlite:///./users.db", echo=False)
# Create tables for User and UserHistory
SQLModel.metadata.create_all(engine)
...
def create_default_user():
    with Session(engine) as session:
        if session.exec(select(User).where(User.username == "user")).first() is None:
            create_user("user", "user", session)
            print("⚡️ Default user/user account created")
...
def register(data: UserCreate, session: Session = Depends(get_session)):
    if session.exec(select(User).where(User.username == data.username)).first():
        raise HTTPException(status_code=400, detail="Username already registered")
    user = create_user(data.username, data.password, session)
    return UserRead(id=user.id, username=user.username)
...
def add_search_history(
    ...
    history = UserHistory(user_id=current_user.id, text=data.text, type="search")
    session.add(history)
    session.commit()
...
def add_view_history(
    ...
    history = UserHistory(user_id=current_user.id, text=data.text, type="view")
    session.add(history)
    session.commit()
    session.refresh(history)
    return {"status": "ok", "id": history.id}
